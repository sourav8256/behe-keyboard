...
public class PCKeyboard extends InputMethodService
    ...
    public void onKey(int primaryCode, int[] keyCodes) {
        ...
        if((primaryCode!=150 && primaryCode!=151 && primaryCode!=152) && ic!=null && ic.getExtractedText(new ExtractedTextRequest(), 0)!=null) {
        ...
        try {
            ...
            for(int i = 0;i<LENGTH;i++){
                if(i<notes.length()) {
                    ...
                    key.label = formatStringforKeypad(suggestions[i]);
                } else {
                    key = keys.get(i);
                    key.label = " ";
                }
